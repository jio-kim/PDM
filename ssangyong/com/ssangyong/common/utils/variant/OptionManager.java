...
/**
 * 옵션 관리 유틸
 * [SR140722-022][20140522] Condition Sorting 관련 수정
 * 
 * @author slobbie
 *
 */
...
public class OptionManager {
	 ...
	 * 
	 * @param bomline
	 * @param bIncludeCorpItem Corp Option Item을 검색하여 옵션셋을 정의해 둘지.. True인 경우, 차후에 Clear()를 반드시 호출해야함.
	 * @throws Exception
	 */
	...
	private void init(boolean bIncludeCorpItem) throws Exception{
		...
		if( bIncludeCorpItem ){
			...
			for( int i = 0; corporateOptionItemIds != null && i < corporateOptionItemIds.length; i++){
				...
				try{
					TCComponent[] corpItems = CustomUtil.queryComponent("Item...", new String[]{"Type", "ItemID"}, new String[]{"Corporate Option Item",corporateOptionItemIds[i]});
		...
		if( parent.hasChildren()){
			...
			for( AIFComponentContext context: contexts ){
				...
				if( parentItemType.equals(TcDefinition.FUNCTION_MASTER_ITEM_TYPE)){
					List conditionList = conditionMap.get(childLine);
				...
				Vector<String[]> userDefineErrorList = new Vector();//사용자 정의 유효성 검사.
	...
	public void usingCheck(TCComponentBOMLine parent, Vector<VariantValue> inUseValueList, HashMap<TCComponentBOMLine, List<String>> conditionMap, TCVariantService variantService) throws TCException{
		...
		if( parent.hasChildren()){
			...
			for( AIFComponentContext context: contexts ){
				...
				String itemType = childLine.getItem().getType();
				if( itemType.equals(TcDefinition.PRODUCT_ITEM_TYPE) ||itemType.equals(TcDefinition.VARIANT_ITEM_TYPE) 
						|| itemType.equals(TcDefinition.FUNCTION_ITEM_TYPE) || itemType.equals(TcDefinition.FUNCTION_MASTER_ITEM_TYPE) 
						|| parentItemType.equals(TcDefinition.FUNCTION_MASTER_ITEM_TYPE)){
	 ...
	 * 
	 * @param parent
	 * @param unUsedValueList 사용자에 의해 unchecked된 VariantValue
	 * @param variantService
	 * @throws TCException
	 */
	public void applyToChild(TCComponentBOMLine parent, Vector<VariantValue> unUsedValueList) throws TCException{
		...
		if( parent.hasChildren()){
			...
			for( AIFComponentContext context: contexts ){
				...
				if( type.equals(TcDefinition.PRODUCT_ITEM_TYPE) || type.equals(TcDefinition.VARIANT_ITEM_TYPE) || 
						type.equals(TcDefinition.FUNCTION_ITEM_TYPE)){
					childLine.refresh();
					Vector<String[]> userDefineErrorList = new Vector();//사용자 정의 유효성 검사.
	...
	private Vector<VariantValue> getVariantValues(TCComponentBOMLine bomLine, Vector<VariantValue> inUseValueList, Vector<String[]> userDefineErrorList) throws TCException{
		...
		List equalErrorList = null;
        ArrayList notUseList = new ArrayList();
        ArrayList notDefineList = new ArrayList();
        Vector<VariantValue> list = new Vector();
		
//		if( lineMvl == null || lineMvl.equals("")) {
//			return list;
//		}
			
//		ConstraintsModel constraintsModel = new ConstraintsModel(bomLine.getItem().getProperty("item_id"), lineMvl, bomLine, variantService);
        ...
        for( String[] errorInfo:errors){
        	if( errorInfo[1] != null && !errorInfo[1].equals("")){
        		if( errorInfo[1].indexOf(VariantValue.TC_MESSAGE_NOT_DEFINE) > -1){
            		...
            		equalErrorList = userDefineErrorList;
            		isUserDefine = true;
            	}
        	}
...
//				이런 경우는 실무에서 발생하면 안됨.
				...
				for( String value : values ){
					...
					VariantValue variantValue = new VariantValue(variantOption, value, (String)descMap.get(value), 
							valueStatus, false);
...
//						condition 추가						
	 ...
	 * @throws TCException
	 */
	public HashMap<TCComponentBOMLine, List<String>> getConditionSetAll(TCComponentBOMLine parent, HashMap<TCComponentBOMLine, List<String>> conditionMap)
			throws TCException{
		...
		if( parent.hasChildren()){
			...
			for( AIFComponentContext context: contexts ){
				...
				if( parentType.equals(TcDefinition.PRODUCT_ITEM_TYPE) ||  parentType.equals(TcDefinition.VARIANT_ITEM_TYPE) 
						|| parentType.equals(TcDefinition.FUNCTION_ITEM_TYPE) || parentType.equals(TcDefinition.FUNCTION_MASTER_ITEM_TYPE)){
	...
	public List<String> getSimpleConditionSet(TCComponentBOMLine line) throws TCException{
		...
		HashMap<String, VariantOption> map = new HashMap();
...
//			사용자 정의가 아닌 경우 element.op 는 모두 "="이다.
	 ...
	 * 
	 * @param data 모델 Data Vector
	 * @return
	 * @throws TCException
	 */
	public ConditionVector getConditionSet(Vector<Vector> data) throws TCException{
		...
		for( int i = 0; i < data.size(); i++){
			...
			VariantOption option = value.getOption();
			SymcConditionElement element = new SymcConditionElement();
		...
		//[20140619] Condition Value Sorting
		ArrayList list = new ArrayList(condition);
		Collections.sort(list);
		condition.clear();
		condition.addAll(list);
		condition.firstElement().ifOrAnd = "";
		for( int i = 1; i < condition.size(); i++){
			SymcConditionElement elm = (SymcConditionElement)condition.get(i);
			elm.ifOrAnd = "AND";
		}
	 ...
	 * 
	 * @param line 현재 선택한 BOM line
	 * @param userDefineErrorList 사용자 정의 메시지(에러)를 담을 객체
	 * @param moduleConstraintList 모듈 옵션 구속 조건을 담을 객체
	 * @param isTarget	현재 선택한 라인이 맞는지 여부
	 * @param bChangeToOrgId	true : 옵션이 정의된 아이템아이디를 입력(true) (Variant OPtion 정의시 사용)
...
//		line.refresh();
		...
		if( type.equals(TcDefinition.PRODUCT_ITEM_TYPE) || type.equals(TcDefinition.VARIANT_ITEM_TYPE) || type.equals(TcDefinition.FUNCTION_ITEM_TYPE)
				|| type.equals(TcDefinition.PRE_PRODUCT_ITEM_TYPE) || type.equals(TcDefinition.PRE_FUNCTION_MASTER_ITEM_TYPE)){
...
//			constraintsModel = new ConstraintsModel(line.getItem().getProperty("item_id"), lineMvl, line, line.getSession().getVariantService());
...
//				이런 경우는 실무에서 발생하면 안됨.
				...
				for( String value : values ){
					...
					if( isTarget ){
						if( valueMap.containsKey(option.option.name + ":" + value)){
							...
							variantValue = new VariantValue(variantOption, value, (String)descMap.get(value), 
									valueStatus, !isTarget);
	...
	/**
	 * 
	 * @param line Corporate 아이템을 BOM라인 변환한것. 
	 * @param isTarget 
	 * @return
	 * @throws TCException
	 */
	private ArrayList<VariantOption> getCorporateOptionSet(TCComponentBOMLine line, boolean isTarget) throws TCException{
        ...
        for(int i = 0; i < bomvariantconfigoutput.configuredOptions.length; i++)
        {
            ...
            for( String valueStr : bomvariantconfigurationoption.modularOption.allowedValues){
            	...
            	VariantValue value = new VariantValue(option, valueStr, (String)descMap.get(valueStr), 
            			valueStatus, !isTarget);
	...
	private ArrayList<TCComponentBOMLine> getBOMLines(TCComponent[] corpItems) throws TCException{
		...
		ArrayList bomlineAL = new ArrayList();
	...
	public void clear(boolean bCurrentWindowClose){
		if( corporateBOMLines == null ) return;
...
//		implements ==> uses(외부) 변경함.
	...
	public static Vector<String[]> getUserDefineErrors(TCComponentBOMLine line, boolean bNeedToParse ) throws TCException{
		String lineMvl = CustomUtil.getTCSession().getVariantService().askLineMvl(line);
//		ConstraintsModel constraintsModel = new ConstraintsModel(line.getItem().getProperty("item_id"), lineMvl, line, CustomUtil.getTCSession().getVariantService());
		ConstraintsModel constraintsModel = new ConstraintsModel(line.getItem().getProperty("item_id"), lineMvl, null, line, CustomUtil.getTCSession().getVariantService());
...
//		ConstraintsModel constraintsModel = new ConstraintsModel(line.getItem().getProperty("item_id"), lineMvl, line, CustomUtil.getTCSession().getVariantService());
		...
		Vector<String[]> moduleConstraintList = new Vector();
        String[][] moduleConstraints = constraintsModel.moduleConstraintsTableData();
        OptionConstraint[] constraints = constraintsModel.moduleConstraints();
        for( String[] moduleConstraint:moduleConstraints){
        	String[] tmpStr = new String[moduleConstraint.length];
	...
	public ModularOption[] getModularOptions(TCComponentBOMLine line) throws TCException {
        ...
        try {
			moduleModel = new ModularOptionModel(treeTable, line, line.hasChildren());
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//            throw new ValidationOptionCombinationException(StringUtil.getTextBundle(registry, "excelErrorNotSetOptions", "MSG", dlgClass));
        ...
        BOMVariantConfigOptionResponse response = variantmanagementservice.getBOMVariantConfigOptions(line.window(), line);
