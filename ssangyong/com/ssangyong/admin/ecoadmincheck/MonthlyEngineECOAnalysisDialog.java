...
public class MonthlyEngineECOAnalysisDialog extends Dialog
{
	...
	{
		super(parent);
		setText("월별 엔진 설계변경 분석(변경사유1,2,3)"); //########## Title
	...
	private void createContents()
	{
		...
		exportExcelButton.addSelectionListener(new SelectionAdapter()
		{
			...
			public void widgetSelected(SelectionEvent e)
			{
				...
				ExportPOIExcel.exportDialog(shell, grid,  "월별 엔진 설계 변경 분석 " + reason.substring(1) , "월별 엔진 설계 변경 분석 " + reason.substring(1), "", null);
			}
		});
...
//		countColumn.setFrozen(true); 수정
		...
		try
		{
			ArrayList<HashMap> resultList = (ArrayList<HashMap>) remoteQuery.execute("com.ssangyong.service.ECOAdminCheckService", "getEngineList", ds);
	...
	protected void search(DataSet ds) throws Exception
	{
		...
		try
		{
			resultList = (ArrayList<HashMap>) remoteQuery.execute("com.ssangyong.service.ECOAdminCheckService", "getMonthlyEngineECOAnalysis", ds);
