		...
		new Job("Load BOM...") {
			...
			protected IStatus run(IProgressMonitor pm) {
				...
				// Root 이하 레벨 구성
					 ...
					 * 
					 * @Override public void run() { ds.put("ecoNo", ecoNo); ds.put("parentNo", parentTreeItem.getText(0)); ds.put("parentRev", parentTreeItem.getText(2)); } }); // Child Item 구성 try { final ArrayList<SYMCBOMEditData> children = (ArrayList<SYMCBOMEditData>) remote.execute("com.ssangyong.service.ECOHistoryService", "selectECOBOMList", ds); if(children == null) { return Status.CANCEL_STATUS; } getShell().getDisplay().syncExec(new Runnable() {
...
//    	return "";
    	...
    	try {
    		if (hmCorpOption == null || hmCorpOption.isEmpty()) {
    			...
    			@SuppressWarnings("unchecked")
    			ArrayList<HashMap<String, String>> alResult = (ArrayList<HashMap<String, String>>) remote.execute("com.ssangyong.service.VariantService", "getVariantValueDesc", ds);
    			if (!(alResult.size() > 0)) {
    				return "";  
    			}
