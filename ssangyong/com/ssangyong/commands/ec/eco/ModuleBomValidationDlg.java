...
import com.ssangyong.commands.ec.eco.module.ModuleBomTableCellRenderer;
import com.ssangyong.commands.ec.eco.module.ModuleBomValidationInfo;
...
//			ds.put("eco_no", ecoNo);
			...
			result = (ArrayList)remote.execute("com.ssangyong.service.ModuleBomValidationService", "validateModule", ds);
...
//			} else 
			...
			if (result.size() > 0){
				result = (ArrayList)remote.execute("com.ssangyong.service.ModuleBomValidationService", "getModuleValidationResult", ds);
			}
		}catch( Exception e){
			waitProgress.dispose();
			throw e;
	...
	private Vector getEpl(boolean bOnlyProbem){
		...
		try{
			...
			ArrayList<SYMCBOMEditData> list = (ArrayList)remote.execute("com.ssangyong.service.ECOHistoryService", "selectECOEplList", ds);
			if( list != null ){
				int epl_no = 1;
				TableItem item = null;
				String mcode_old = "";
