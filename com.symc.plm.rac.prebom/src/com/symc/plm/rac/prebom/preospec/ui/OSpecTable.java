...
public class OSpecTable {
	...
	private boolean isFixedValueEditable = false;
	private boolean isValueEditable = false;
	private HashMap<String, ArrayList<String>> simpleDataMap = null;
	
	public OSpecTable(OSpec ospec, Vector<Vector> customData, boolean isFixedValueEditable, boolean isValueEditable) throws Exception{
		this.ospec = ospec;
		this.isFixedValueEditable = isFixedValueEditable;
		this.isValueEditable = isValueEditable;
		this.ospecTablePanel = getOspecTable(customData);
	}
	
	public OSpecTable(OSpec ospec, Vector<Vector> customData) throws Exception{
		this(ospec, customData, false, false);
	}
	...
	private JPanel getOspecTable(Vector<Vector> customData) throws Exception{
	    ...
	    scroll.setCorner(JScrollPane.UPPER_LEFT_CORNER, fixedOspecViewTable
	        .getTableHeader());			
	...
	private void checkSelection(boolean isFixedTable) {
		...
		if (fixedSelectedIndex != selectedIndex) {
			if (isFixedTable) {
				ospecViewTable.setRowSelectionInterval(fixedSelectedIndex,
						fixedSelectedIndex);
			} else {
				fixedOspecViewTable.setRowSelectionInterval(selectedIndex,
						selectedIndex);
			}
		}
	}
	
	public static Vector getData(OSpec ospec, ArrayList<String> opValueList){
...
//					Component com = super.getTableCellRendererComponent(jtable, obj, flag, flag1, i, j);
//					if( com instanceof JLabel){
//						JLabel label = (JLabel)com;
//						label.setToolTipText((String)jtable.getValueAt(i, j));
//					}
//					return com;
//				}
//	    		
...
//	    }
	    ...
	    if( bIsFixedTable ){
	    	...
	    	ColumnGroup releasedDate = new ColumnGroup(
	    			ospec.getReleasedDate().substring(0, 4) + "-"
	    			+ ospec.getReleasedDate().substring(4, 6) + "-"
...
//	public Vector<Vector> minus2(Vector<Vector> data, Vector header, boolean bFillEmpty){
//		
//		Vector trimHeader = getTrimHeader();
//		Vector<Vector> result = null;
//		if( this.data != null ){
//			result = new Vector(){
//
