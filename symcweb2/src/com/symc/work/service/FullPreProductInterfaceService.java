...
import com.kgm.commands.ospec.op.StoredOptionSet;
 ...
 */
public class FullPreProductInterfaceService {
    ...
    public static String[] bomlineProperties =
        {
            ...
            //[SR170706-008][LJG] Proto Tooling 컬럼 추가
            TcConstants.PROP_BL_PROTO_TOOLING,   //22
        ...
        try
        {
        	...
        	if( preferences != null ){
	        	...
	        	for( int i =0; filterPreProductIdList != null && i < filterPreProductIdList.length ; i++){
	        		...
	        		includeProductValues = includeProductValues + (i != 0 ?";":"") + filterPreProductIdList[i];
	        	
	        	}
        	}
        	System.out.println( "----------------  excludeProductValues :" + includeProductValues);
        	String[] entries = null;
        	String[] values = null;
        	
        	//필요한 프로젝트만 지정해서 돌릴 때는 아래를 사용할 것.
//        	includeProductValues = "PC300PRE;PD300PRE;PE100PRE;PG15DTF";

        	if( includeProductValues != null && !includeProductValues.equals("") && includeProductValues.length() > 0 ){ 
	        	
        		entries = new String[]{"Gate", "ID"};
	            values = new String[]{"6;7;8;9;10", includeProductValues };
        	
        	}else{
        		
        		entries = new String[]{"Gate"};
	            values = new String[]{"6;7;8;9;10"};
...
//            List<KeyLOV> systemCodeLovLists = (List<KeyLOV>) TcCommonDao.getTcCommonDao().selectList("com.symc.tc.lov.getLOVValues", "S7_SYSTEM_CODE");
            ...
            /**
             * FullList에서 제외시켜야 할 대상은 
             * 1. SEARCH_PREPRODUCTREV_INTERFACE query를 수정해서 ITEM_ID != ''로 추가 조건 추가
             * */
            ...
            //필요한 프로젝트만 지정해서 돌릴 때는 아래를 사용할 것.
//            ModelObject []findPreProducts = queryService.searchTcObject(TcConstants.SEARCH_PREPRODUCTREV, new String[]{"ID"}, new String[]{"PC300PRE"}, new String[]{TcConstants.PROP_ITEM_ID, TcConstants.PROP_S7_OSPEC_NO, TcConstants.PROP_ITEMS_TAG});
...
//            dataService.getProperties(itemList.toArray(new ModelObject[0]), new String[]{"revision_list"});
            for (Item prodItem : itemList)
            {
                ...
                // 1. Ospec 을 읽어온다.
                ModelObject ospecRev = getOSpecRevision(prodRevision);
...
//                        BOMLine[] fmpLines = strService.getExpandPSOneLevel(tcSession, funcLine);
    ...
    private ModelObject getOSpecRevision(ItemRevision preProduct) throws Exception
    {
        try
        {
            ...
            if( ospecNo == null || ospecNo.getStringValue() == null || ospecNo.getStringValue().equals("")){
    		                        ...
    		                        StringBuffer logBuff, int func_idx, int fmp_idx, int line_idx, int level_idx, IFMasterFullService dbService) throws Exception
    {
        try
        {
            ...
            for (ModelObject child : children)
            {
                ...
                //childRevision.get_current_id_uid();
...
//                    propMap.put("PRT_TOOLG_INVESTMENT", currentRevision.getPropertyObject(TcConstants.PROP_S7_PRT_TOOLG_INVESTMENT).getDisplayableValue());
            ...
            //KCH 추가 - SYSTEM_ROW_KEY Query 변경
            /**
             *  20180528  SYSTEM_ROW_KEY가 NULL 이면 EXCEPTION발생 하도록 변경 함....  
             *  laho been
             */
//            String tmpSystemRowKey = bomLine.getPropertyObject(TcConstants.PROP_BL_SYSTEM_ROW_KEY).getStringValue();
//            if( tmpSystemRowKey == null || "".equals(tmpSystemRowKey) )
//            	throw new Exception("SYSTEM_KEY_IS_NULL : " + propMap );
//            
//            propMap.put("SYSTEM_ROW_KEY", tmpSystemRowKey );
