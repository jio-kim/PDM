...
public class SYMCRemoteUtil {
	 ...
	 * @since : 2012. 12. 13.
	 */
	public SYMCRemoteUtil() {
		try {
			//VM parameter에 옵션 값 추가하여 변경. 소스 변경 없이 진행.
			// 옵션 값 : -DisLocalWebSQL=true
			boolean isLocal = false;
			String isLocalWebSQL = System.getProperty("isLocalWebSQL");
			if(isLocalWebSQL != null)
			{
				isLocal = isLocalWebSQL.equalsIgnoreCase("true");
			}

			if (isLocal) {
				this.strAppletHomeUrl = "http://localhost:7090/ssangyongweb/HomeServlet";
			} else {
				Registry registry = Registry.getRegistry("site_specific");
				String portalWebServer = registry.getString("portalWebServer");
				this.strAppletHomeUrl = portalWebServer + ":7090/ssangyongweb/HomeServlet";
			}
	 ...
	 * @param hostURL
	 */
	public SYMCRemoteUtil(String servletUrl) {
