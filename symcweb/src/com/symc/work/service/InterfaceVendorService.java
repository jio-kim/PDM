...
public class InterfaceVendorService {
    ...
    public Object startInterfaceVendorService() throws Exception {
        try
        {
            ...
            for (HashMap<String, String> ret : retList)
            {
                ...
                String sysCode = ret.get("SYSTEM_CODE");
            ...
            tcItemUtil.getProperties(targetRevList.toArray(new ModelObject[0]), new String[]{TcConstants.PROP_ITEM_ID, TcConstants.PROP_S7_PROJCODE, TcConstants.PROP_S7_BUDGETCODE, TcConstants.PROP_RELEASE_STATUS_LIST, TcConstants.PROP_OWNING_USER, TcConstants.PROP_OWNING_GROUP});
            ...
            */
            ...
            for (String mapKey : sysGrouppingRevision.keySet())
            {
             // CCN을 생성한다.
                ...
                HashMap<String, HashMap<String, Object>> revVendorMap = getAllRevisionVendors(sysGrouppingRevision.get(mapKey));
                ...
                try
                {
                    for (ItemRevision rev : sysGrouppingRevision.get(mapKey))
                    {
                        ...
                        propMap.put(TcConstants.PROP_S7_SELECTED_COMPANY, revVendorMap.get(itemId).get("COST"));
                        tcItemUtil.setAttributes(newRev, propMap);
                        ...
                        revInfoMap.put(TcConstants.PROP_S7_SELECTED_COMPANY, revVendorMap.get(itemId).get("COST"));
                        if (ccnRev != null)
            ...
            if (null != ccnList && ccnList.size() > 0) {
                tcItemUtil.createNewProcess(ccnList.toArray(new ModelObject[0]), "Self Release for Update Vendor", "CSR");
                ccnUtil.insertCCNInformation(ccnAttachedItemList);
                ccnUtil.insertCCNMaster(ccnList, "vendor");
                // [20160913][ymjang] CCN IF 생성 기능 프로시저로 이관함.
                ccnUtil.insertIfCCN(ccnList, "vendor");
            }

            // mark to Interface Table Success
            updateInterfaceTable("com.symc.costvendor.updateVendorInterfaceTable", ccnAttachedItemList);
    ...
    private void updateInterfaceTable(String sql, HashMap<String, HashMap<String, Object>> ccnAttachedItemList) throws Exception
    {
        try
        {
            ...
            for (String key : ccnAttachedItemList.keySet())
            {
                ...
                updateMap.put("UPDATE_MSG", "Completed");
    ...
    private HashMap<String, HashMap<String, Object>> getAllRevisionVendors(ArrayList<ItemRevision> revList) throws Exception
    {
        ...
        try
        {
            for (ItemRevision rev : revList)
            {
                ...
                List<HashMap<String, Object>> retList = (List<HashMap<String, Object>>) dbDao.selectList("com.symc.costvendor.getDatasForUpdateVendor", inputMap);
    ...
    private ItemRevision createCCNRevision(String projCode, String sysCode) throws Exception
    {
        try
        {
            ...
            ccnPropMap.put(TcConstants.PROP_S7_MATERIALCOSTCHANGE, false);
            ccnPropMap.put(TcConstants.PROP_S7_THEOTHERS, true);
            ccnPropMap.put(TcConstants.PROP_OBJECT_DESC, "modify Vendor");
...
//            newIdMap.put("TO_LEN", 12);
            ...
            Item ccnItem = tcItemUtil.getItem("S7_PreCCN", newCCNID);
            if (ccnItem != null)
            {
    ...
    {
        return (List<HashMap<String, String>>) dbDao.selectList("com.symc.costvendor.getIdListForUpdateVendor");
    }
