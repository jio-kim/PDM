...
//  private TcQuery  tcQuery;    
     ...
     */
    protected ModelObject createDataSetForNamedReference(String datsetname, 
            String datsettype, 
...
//      NamedReferenceObjectInfo[] nrObjectInfos = pnrObjectInfos;
    ...
    public DatasetInfo3[] createDatasetInfo3(ModelObject ref_obj, 
                                             HashMap<String, String> datasetProps, 
...
//  }
    ...
    public Dataset[] retrieveDatasetObjects (ItemRevision itemRevision) throws Exception { 
        ...
        ServiceData serviceData = tcItemUtil.getProperties(datasets,  new String[]{PropertyConstant.ATTR_NAME_DATASET_TYPE, PropertyConstant.ATTR_NAME_DATASET_TYPE_NAME, "object_name"});
...
//      com.teamcenter.services.strong.core._2006_03.DataManagement.CreateDatasetsResponse createdatasetresponse = null;
        ...
        tcServiceManager.getDataService().getProperties(new ModelObject[] {rev}, new String[] {PropertyConstant.ATTR_NAME_ITEMID, PropertyConstant.ATTR_NAME_ITEMREVID});        
...
//      datasetProps.put("name", TcItemUtil.getDataSetName(rev.get_item_id(), rev.get_item_revision_id(), preDataset.get_object_name(), true));        
      // 이전 리비전 정보가 없으면 디폴트 데이터셋 생성 룰을 따른다. (최초생성 시)
        ...
        String strCadPartRev = this.getCadPartRev(cadRevId);
...
//        datasetProps.put("toolUsed", TcConstants.TOOL_NX_UGMASTER);
        ...
        if(!tcServiceManager.getDataService().ServiceDataError(createdatasetresponse.servData)){             
            ...
            throw new Exception(TcUtil.makeMessageOfFail(createdatasetresponse.servData).get(TcMessage.TC_RETURN_FAIL_REASON).toString());            
        }
    }
    ...
    private String getCadPartRev(String strCadPartRev) {
        strCadPartRev = StringUtil.nullToString(strCadPartRev); 
    ...
    private void deleteDataset(Dataset dataset, String[] otherSideObjectTypes, String relationTypeName) throws Exception {        
        ...
        if(!tcServiceManager.getDataService().ServiceDataError(relationResp.serviceData))
        {
            ...
            throw new Exception(TcUtil.makeMessageOfFail(relationResp.serviceData).get(TcMessage.TC_RETURN_FAIL_REASON).toString());
        }
...
//    }
    ...
    public  ModelObject[] getReferencedListFromDataset (Dataset[] dataset, String[] reference_names) throws Exception {
        List<ModelObject> fileCompList = new ArrayList<ModelObject>();

        tcServiceManager.getDataService().getProperties(dataset, new String[] {PropertyConstant.ATTR_NAME_REF_LIST, PropertyConstant.ATTR_NAME_REF_NAMES});

        ModelObject[] reflistComps = dataset[0].get_ref_list();
        ...
        if( reference_names.length > 0) {
            for(String reference_name : reference_names) {
                for(int i=0;  i< idx_names.length; i++){
                        if(idx_names[i].equals(reference_name)){
                            fileCompList.add(reflistComps[i]);
                        }
                }
            }
        }else{
           for(ModelObject tmpComp : reflistComps) {
                fileCompList.add(tmpComp);
           }
        }
        return fileCompList.toArray(new ModelObject[fileCompList.size()]);
    }   
    ...
    public  ModelObject[] getReferencedQuickAccessBinaryListFromDataset (Dataset[] dataset) throws Exception {
        ...
        for(int i=0; i<idx_names.length; i++) {
            if(idx_names.equals(TcConstants.RELATION_QUICK_ACCESS_BINARY_REFERENCES)){
                fileComps[i] = reflistComps[i];
            }
        }
    ...
    public String getReferenecedTypeFromDataset (Dataset dataset, ImanFile ifile) throws Exception {
        ModelObject[] reflistComps = null;
        ModelObject[] fileComps = new ModelObject[] {dataset};
        tcServiceManager.getDataService().getProperties(fileComps, new String[] {PropertyConstant.ATTR_NAME_REF_LIST, PropertyConstant.ATTR_NAME_REF_NAMES});
...
//            ldataResult.setString("sDataSetPuid", dataset_puid);
    ...
    public void createRelation( ModelObject parent, ModelObject child, String sRelationType) throws Exception {        
        ...
        if(tcServiceManager.getDataService().ServiceDataError(createOrUpdateRelationsResponse.serviceData)){
            throw new Exception(TcUtil.makeMessageOfFail(createOrUpdateRelationsResponse.serviceData).get(TcMessage.TC_RETURN_FAIL_REASON).toString());
      }
    }
