...
import com.ssangyong.soa.common.util.ContextUtil;
...
com.teamcenter.services.internal.strong.core._2010_09.FileManagement
{
    ...
    public TcFileManagementService(Session tcSession) throws ServiceException, Exception {
        ...
        if(this.fileManagementUtility == null) {
			...
			fscMap = loadFscInfo();
            this.fileManagementUtility = new FileManagementUtility(tcSession.getConnection(),(String)fscMap.get("hostname"), (String[])fscMap.get("fscUrl"), (String[])fscMap.get("bootStrapUrl"), (String)fscMap.get("target_dir"));
        }
...
//      fscMap.put("fscUrl", fscUrl);
        ...
        String cacheDownloadDir = "";
        try {
            Properties contextProperties = (Properties)ContextUtil.getBean("contextProperties");
            cacheDownloadDir = contextProperties.getProperty("tc.cacheDownloadPath");    
        }catch(Exception e) {
            e.printStackTrace();
        }   cacheDownloadDir = "c://temp";
