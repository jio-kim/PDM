 ...
 *
 */
public class SearchECODialog {
	
	private Shell shell;
	/** 검색 조건 */
	private Text ecoIDText, creatorText;
	
	private Button searchButton;
	private Table resultTable;
	
	/** 테이블 헤더 */
	private String[] tableColumns = SYMCECConstant.ECO_CONCURRENTECO_TABLE_COLS;
	...
	private void createSearchLayout() {
		...
		lbl_ecoID.setText("ECO NO. : ");
		
		ecoIDText = new Text(composite, SWT.BORDER);
		ecoIDText.addKeyListener(new KeyAdapter() {
			@Override
	...
	private void searchAction() {
		String itemID = ecoIDText.getText();
		String creatorName = creatorText.getText();
		if(searchValues[0].equals(itemID) && searchValues[1].equals(creatorName) && resultTable.getItemCount() > 0 )  return;
	...
	private void search() {
		try {
			TCComponent[] results= CustomUtil.queryComponent("__SYMC_S7_ECO_Revision", searchConditions, searchValues);
			if(results == null) return;
			for (int i = 0; i < results.length; i++) {
				TCComponentItemRevision ecoRevision = (TCComponentItemRevision) results[i];
				String[] properties = ecoRevision.getProperties(wantProperties);
				TableItem rowItem = new TableItem(resultTable, SWT.NONE);
				rowItem.setText(properties);
				rowItem.setData("TCComponentItemRevision", ecoRevision);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
