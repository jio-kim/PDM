...
//	private String type = "";
	...
	public OptionSetOperation(ArrayList<VariantOption> globalOptionSet, ArrayList<VariantOption> enableOptionSet, 
	...
	public OptionSetOperation(ArrayList<VariantOption> globalOptionSet, ArrayList<VariantOption> enableOptionSet, 
			ArrayList<VariantOption> selectedLineOptionSet, TCComponentBOMLine selectedLine, 
			Vector<String[]> userDefineErrorList,Vector<String[]> moduleConstraintList, OptionManager manager, ArrayList<VariantOption> optionSetToDelete,
			Vector<Vector> allData, JDialog parent, WaitProgressBar waitProgress, String type){
		this.globalOptionSet = globalOptionSet;
		this.enableOptionSet = enableOptionSet;
		this.selectedLineOptionSet = selectedLineOptionSet;
		this.selectedLine = selectedLine;
	...
	*/
	...
	public OptionSetOperation(TCComponentBOMLine selectedLine, 
	...
	public void executeOperation() throws Exception {
		try{
//			System.out.println("option set operation start : " + new Date());
...
//				}
//				System.out.println("option set operation end : " + new Date());
//			} else {
				if( operationType == OPTION_SAVE){
//					System.out.println("2");
					optionApply();
...
//			}
	...
	private void optionApply() throws TCException{
		...
		//tmpList에 담아도 후처리가 주석이 막혀있어 사용처가 없음....  속도만 잡아먹고 있어서 이것도 주석처리함.
...
//		}
	...
	/*
	private void optionApply_() throws TCException{
		
		waitProgress.setStatus("Option updating...", true);
		apply_(allData, userDefineErrorList, selectedLine);
...
//		System.out.println("option set operation apply start : " + new Date());
		...
		if( targets[0] instanceof TCComponentBOMLine){
			try{
				...
				int curNum = 0;
				//설정된 옵션 코드값을 확인합니다.
//				System.out.println("설정된 옵션 코드값을 확인 시작 : " + new Date());
...
//						System.out.println("옵션 추가 시작 (" + s + ") : " + new Date());
						try{
...
//						System.out.println("옵션 추가 완료 (" + s + ") : " + new Date());
...
//				System.out.println("설정된 옵션 코드값을 확인 완료 : " + new Date());
				
			    //개선 전
				/*
				selectedLine.window().save();
				System.out.println("설정된 옵션 코드값을 확인 완료 save : " + new Date());
				selectedLine.refresh();
				System.out.println("설정된 옵션 코드값을 확인 완료 refresh : " + new Date());
				*/
...
//				System.out.println("기존에 있던 옵션값을 모두 뺀 경우는 해당 옵션 제거 시작 : " + new Date());
...
//		        System.out.println("notUseErrorcheck 시작 : " + new Date());
...
//				System.out.println("notDefineErrorcheck 시작 : " + new Date());
...
//		        System.out.println("userDefineErrorcheck 시작 : " + new Date());
		        //사용자 정의 오류 체크
...
//		        System.out.println("moduleConstraintCheck 시작 : " + new Date());
		        //내부 모듈 구속 조건
		        ...
		        try{
		        	...
		        	System.out.println("VariantErrorCheck 적용 시작 : " + new Date());
			        ...
			        System.out.println("VariantErrorCheck 적용 완료 : " + new Date());
		        }catch(TCException tce){
		        ...
		        */
			    ...
			    //개선 후 (정상 처리시에만 window save하는 걸로....)
...
//		        System.out.println("VariantErrorCheck 적용 완료 : " + new Date());
		        ...
		        selectedLine.window().save();
//		        ((SYMCBOMWindow)selectedLine.window()).save_super();
		        selectedLine.refresh();
		        waitProgress.setStatus("BOMWindow Save 완료", true);
//		        System.out.println("BOMWindow Save 완료 : " + new Date());
			}catch( TCException e){
...
//				selectedLine.refresh();
	...
	/*
	private void apply_(Vector<Vector> allData, Vector<String[]> userDefineErrorList, TCComponentBOMLine selectedLine) throws TCException{
...
//						condition 추가						
				...
				((SYMCBOMWindow)selectedLine.window()).save_();
				selectedLine.refresh();
