...
//					item.bomEditData.setChgDesc(value);//[20200113 CSH]값을 set 하면 동일 Value 두번 적용하면 Modified로 인지하지 않음. 주적 처리
	...
	private void setTableData() throws Exception {
		...
		final ArrayList<SYMCBOMEditData> rows = (ArrayList<SYMCBOMEditData>) remote.execute("com.kgm.service.ECOHistoryService", "selectECOEplList", ds);
...
//							System.out.println("  > "+item.getText(CHANGE_DESC));
					...
					try {
						...
						Boolean updateStatus = (Boolean) remote.execute("com.kgm.service.ECOHistoryService", "updateECOEPLProperties", ds);
