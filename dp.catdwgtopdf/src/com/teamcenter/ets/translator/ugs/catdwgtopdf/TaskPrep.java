...
public class TaskPrep extends DefaultTaskPrep {
   private static Registry registry = Registry.getRegistry("com.teamcenter.ets.translator.ugs.catdwgtopdf.catdwgtopdf");
...
//      Connection connection = new Connection(hostPath, new HttpState(), credentialMgr, SoaConstants.REST, SoaConstants.HTTP, true);
      ...
      for (int i = 0; i < primary_objs.length; i++) {
         ...
         if (!dataset.getTypeObject().getName().equalsIgnoreCase("CATDrawing")) {
            continue;
         }
...
//        			 files[0] = (ImanFile) refObjs[j];
         ...
         zTransTask = addRefIdToTask(prepTransTask(zTransTask, dataset, itemRev, exportFilename, true, true, ".pdf", 0, null), i);
      }
