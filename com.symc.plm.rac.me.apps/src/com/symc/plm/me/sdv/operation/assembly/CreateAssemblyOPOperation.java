 ...
 * 
 */
public class CreateAssemblyOPOperation extends AbstractSDVActionOperation {
     ...
     * @param dataSet
     */
    public CreateAssemblyOPOperation(int actionId, String ownerId, IDataSet dataSet) {
     ...
     * @param isIFExcution
     */
    public CreateAssemblyOPOperation(int actionId, String ownerId, IDataSet dataSet, boolean isIFExcution) {
        super(actionId, ownerId, dataSet);
    ...
    public void endOperation() {
        ...
        if (isAbortRequested() || tempLineBOMLine == null)
            return;
        ...
        try {
            ...
            if (actionId == 1)
                ...
                Dialog dialog = (Dialog) UIManager.getAvailableDialog("symc.me.bop.CreateAssemblyOPDialog");
                shell = dialog.getShell();
            }

            String message = registry.getString("OPAddedToTempLine.MSG", "Opearation(%0) was added to Temp Line(%1).").replace("%0", createdItem.getProperty(SDVPropertyConstant.ITEM_ITEM_ID)).replace("%1", tempLineBOMLine.getProperty(SDVPropertyConstant.BL_ITEM_ID));
            MessageBox.post(shell, message, "Information", MessageBox.INFORMATION);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
     ...
     * @see
     */
    public TCComponentBOMLine createOperation() throws Exception {
        dataSet = getDataSet();
