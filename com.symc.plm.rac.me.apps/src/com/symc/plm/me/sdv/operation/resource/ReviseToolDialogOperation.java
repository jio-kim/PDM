 ...
 * 
 */
public class ReviseToolDialogOperation extends AbstractTCSDVOperation implements IDialogOpertation {

    private static final Logger logger = Logger.getLogger(ReviseToolDialogOperation.class);
    ...
    protected boolean validateItemRevision(TCComponentItemRevision targetItemRevision) throws Exception {
        ...
        // 공구 Item이 아닌 경우
        if (!itemType.equals(SDVTypeConstant.BOP_PROCESS_TOOL_ITEM_REV)) {
            throw new Exception(registry.getString("ToolItem.Check.MSG"));
        }

        // 조립공구검색시
        if (bopType.subSequence(0, 1).equals(registry.getString("BOP.Type.Assy"))) {
            if (!typeKey.equals("A") && !typeKey.equals("E") && !typeKey.equals("H")) {
