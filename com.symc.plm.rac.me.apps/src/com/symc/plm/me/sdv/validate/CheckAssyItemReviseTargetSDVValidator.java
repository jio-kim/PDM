 ...
 *
 */
public class CheckAssyItemReviseTargetSDVValidator implements ISDVValidator {
	private Registry reviseRegistry = Registry.getRegistry(CheckAssyItemReviseTargetSDVValidator.class);
	...
	public void validate(String commandId, Map<String, Object> parameter,
		...
		try
		{
			for (InterfaceAIFComponent targetComponent : selectedTargets)
			{
        		...
        		List<String> bodyTypeList = Arrays.asList(reviseRegistry.getStringArray("Property.AssyBOP.Type"));
				if ((! bodyTypeList.contains(latestRevision.getProperty(SDVPropertyConstant.SHOP_REV_PROCESS_TYPE).toUpperCase()) &&
					    ! latestRevision.getItem().getType().equals(SDVTypeConstant.BOP_PROCESS_ASSY_OPERATION_ITEM)) ||
					latestRevision.getItem().getType().equals(SDVTypeConstant.BOP_PROCESS_SHOP_ITEM))
				{
					throw new Exception(reviseRegistry.getString("SelectAssyItemTarget.MESSAGE", "Please select Assy BOP Item."));
				}
