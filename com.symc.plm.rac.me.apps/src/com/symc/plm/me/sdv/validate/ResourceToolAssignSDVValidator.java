...
public class ResourceToolAssignSDVValidator implements ISDVValidator {

    private Registry registry = Registry.getRegistry(ResourceToolAssignSDVValidator.class);

    public ResourceToolAssignSDVValidator() {
    }
            ...
            try {
                ...
                if(!targetType.equals(SDVTypeConstant.BOP_PROCESS_ASSY_OPERATION_ITEM) && !targetType.equals(SDVTypeConstant.BOP_PROCESS_BODY_OPERATION_ITEM) && !targetType.equals(SDVTypeConstant.BOP_PROCESS_PAINT_OPERATION_ITEM)) {
                    for (InterfaceAIFComponent aifComponent : resourceItemsList) {
                        ...
                        if(resourceType.equals(SDVTypeConstant.BOP_PROCESS_TOOL_ITEM_REV)) {
                            throw new ValidateSDVException(registry.getString("ResourceToolAssign.Operation.MESSAGE").replace("%0", resourceItemId));                            
                        }
                    }
                }                
            } catch (Exception ex) {
