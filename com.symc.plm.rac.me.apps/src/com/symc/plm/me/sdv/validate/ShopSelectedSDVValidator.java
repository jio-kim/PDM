 ...
 * 
 */
public class ShopSelectedSDVValidator implements ISDVValidator {
    private Registry registry = Registry.getRegistry(ShopSelectedSDVValidator.class);
    ...
    public void validate(String commandId, Map<String, Object> parameter, Object applicationCtx) throws SDVException {
        ...
        if(selectedTargets.length == 0) {
            throw new ValidateSDVException(registry.getString("Common.SelectedComponent.MESSAGE").replace("%0", "SHOP"));
        }
        if (selectedTargets.length > 1) {
            throw new ValidateSDVException(registry.getString("Common.SelectedOne.MESSAGE.MSG").replace("%0", "SHOP"));
        }
        try {
