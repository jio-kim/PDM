...
public class SYMTcUtil {
    ...
    private static HashMap<String, PropertyContentProvider> propertyProviderMap;
...
//            itemrevision = cItemRevisionType.findRevision(itemId, revision);
    ...
    /**
     * BOP 공법(Operation) Activity 리스트를 가져온다.
     * 
     ...
     * @see
     */
    public static List<HashMap<String, Object>> getActivityList(TCComponentMfgBvrOperation bvrOperation) throws Exception {
        List<HashMap<String, Object>> activityList = new ArrayList<HashMap<String, Object>>();
        if (bvrOperation == null) {
            return activityList;
        }
                ...
                if (children != null) {
                    for (int i = 0; i < children.length; i++) {
                        if (children[i] instanceof TCComponentMEActivity) {
                            ...
                            int seq = (i + 1) * 10;
                            //010, 020 -> 100단위 자리로 SEQ 생성
                            activityMap.put("SEQ", (seq < 100) ? "0" + seq : "" + seq);
                            String activityName = child.getProperty(SDVPropertyConstant.ITEM_OBJECT_NAME);
                            String activityNameEng = child.getProperty(SDVPropertyConstant.ACTIVITY_ENG_NAME);
                            activityMap.put(SDVPropertyConstant.ITEM_OBJECT_NAME, activityName);
                            activityMap.put(SDVPropertyConstant.ACTIVITY_ENG_NAME, activityNameEng);
                            activityMap.put(SDVPropertyConstant.ACTIVITY_SYSTEM_CODE, child.getProperty(SDVPropertyConstant.ACTIVITY_SYSTEM_CODE));
                            activityMap.put(SDVPropertyConstant.ACTIVITY_SYSTEM_CATEGORY, child.getProperty(SDVPropertyConstant.ACTIVITY_SYSTEM_CATEGORY));
                            ...
                            TCComponentBOMLine[] tools = child.getReferenceTools(bvrOperation);
                            if (tools != null && tools.length > 0) {
                                activityMap.put(SDVPropertyConstant.ACTIVITY_TOOL_LIST, tools);
                            }
                            activityMap.put(SDVPropertyConstant.ACTIVITY_CONTROL_POINT, child.getProperty(SDVPropertyConstant.ACTIVITY_CONTROL_POINT));
                            activityMap.put(SDVPropertyConstant.ACTIVITY_CONTROL_BASIS, child.getProperty(SDVPropertyConstant.ACTIVITY_CONTROL_BASIS));
                            activityMap.put(SDVPropertyConstant.ACTIVITY_WORK_OVERLAP_TYPE, child.getProperty(SDVPropertyConstant.ACTIVITY_WORK_OVERLAP_TYPE));
     ...
     * @date 2013. 2. 22.
     * @param String
     *            itemId, String revisionId, ArrayList<String> filterType, String strDownloadPath, String ecoNo
    ...
    public static HashMap<String, File[]> getDatasetFiles(String itemId, String revisionId, ArrayList<String> filterType, String strDownloadPath, String ecoNo) throws Exception {
        if ("".equals(BundleUtil.nullToString(itemId))) {
            return null;
        }
        if ("".equals(BundleUtil.nullToString(revisionId))) {
            return null;
        }
     ...
     *            itemRevision, ArrayList<String> filterType, String strDownloadPath, String ecoNo
     * @return HashMap<String,File[]>
     * @exception
     * @throws
     * @see
    ...
    public static HashMap<String, File[]> getDatasetFiles(TCComponentItemRevision itemRevision, ArrayList<String> filterType, String strDownloadPath, String ecoNo) throws Exception {
        ...
        for (int i = 0; datasets != null && i < datasets.size(); i++) {
            ...
            // 필터 조건이 존재하면 필터에 존재하는 Dataset만 가져온다.
            if (filterType == null || (filterType != null && filterType.contains(dataSet.getType()))) {
                if (dataSet != null && ("".equals(BundleUtil.nullToString(ecoNo)) || dataSet.getProperty("s7_ECO_NO").equals(ecoNo))) {
                    // TCComponentTcFile[] imanFile = dataSet.getTcFiles();
                    TCComponentTcFile[] imanFile = getImanFile(dataSet);
                    if (imanFile == null || imanFile.length == 0) {
                        datasetFiles.put(dataSet.getType(), null);
     ...
     * @date 2013. 4. 3.
     * @param String
     *            [] ecos, String strDownloadPath, ArrayList<String> filterType
    ...
    /**
     * ItemID를 가지고 TCComponentItem를 검색한다.
     * 
     * @method getLatestedRevItem
     * @date 2013. 11. 26.
     * @param
     ...
     * @see
     */
    public static TCComponentItemRevision getLatestedRevItem(String itemID) throws Exception {
        TCComponentItem findItem = getItem(itemID);
        if(findItem == null) {
            return null;
        }        
        return findItem.getLatestItemRevision();
    }
    ...
    /**
     * Teamcenter에서 등록된 Property의 Display Name을 리턴하는 함수
     * 
     * @param typeName
     *            조회할 Component의 Type
     * @param propName
     ...
     * @return
     */
    public static String getPropertyDisplayName(String typeName, String propName) throws Exception {
        if (propertyProviderMap == null) {
            propertyProviderMap = new HashMap<String, PropertyContentProvider>();
        }

        try {
            if (!propertyProviderMap.containsKey(typeName)) {
                propertyProviderMap.put(typeName, new PropertyContentProvider(typeName));
            }

            PropertyContentProvider provider = propertyProviderMap.get(typeName);

            if (provider.containsKey(propName)) {
                return provider.get(propName);
            }

            TCSession session = (TCSession) AIFUtility.getSessionManager().getDefaultSession();
            TCComponentType itemType = session.getTypeComponent(typeName);
            String displayName = propName;
            for (TCPropertyDescriptor propDesc : itemType.getPropertyDescriptors()) {
                String pname = propDesc.getName();
                if (propName.equals(pname))
                    displayName = propDesc.getDisplayName();
                provider.put(propDesc.getName(), propDesc.getDisplayName());
            }
            // TCPropertyDescriptor propDesc = itemType.getPropertyDescriptor(propName);
    ...
    public static boolean getRevisionHasDrawingDataset(TCComponentItemRevision rev) throws Exception {
        ...
        for (int i = 0; i < context.length; i++) {
            ...
            if (childComponent.isTypeOf(TcDefinition.DATASET_TYPE_ACAD) || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_CGM) || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_EDIF) || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_ORCAD) || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_PROASM) || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_PRODRW)
                    || childComponent.isTypeOf(TcDefinition.DATASET_TYPE_PROPRT)) {
                return true;
            }
        }
    ...
    /**
     * BOP BOMWindow TopLine을 가지는 BOPBOMWindow 가져온다.
     * 
     * @method TCComponentBOPWindow
     * @date 2013. 11. 27.
     * @param
     ...
     * @see
     */
    public static TCComponentBOPWindow getTopBOPBOMWindow(TCComponentItemRevision revision) throws TCException {
        TCSession session = revision.getSession();
        TCComponentBOPWindowType type = (TCComponentBOPWindowType) session.getTypeComponent("BOPWindow");
        TCComponentRevisionRuleType type2 = (TCComponentRevisionRuleType) session.getTypeComponent("RevisionRule");
        TCComponentRevisionRule rule = type2.getDefaultRule();
        TCComponentBOPWindow bopWindow = (TCComponentBOPWindow) type.create(rule);
        bopWindow.setWindowTopLine(null, revision, null, null);
        return bopWindow;
    }

    public static TCComponentBOPWindow getTopBOPBOMWindow(TCComponentItemRevision revision, String ruleName) throws Exception {
        TCSession session = revision.getSession();
        ...
        // TCComponentRevisionRuleType type2 = (TCComponentRevisionRuleType) session.getTypeComponent("RevisionRule");
        // TCComponentRevisionRule rule = type2.getDefaultRule();
        TCComponentBOPWindow bopWindow = (TCComponentBOPWindow) type.create(revRule);
        bopWindow.setWindowTopLine(null, revision, null, null);
        return bopWindow;
    }
...
//        boolean isInProcess = !components.getProperty("process_stage_list").equalsIgnoreCase("");
    ...
    /**
     * Latest Revision의 Released 상태를 알아낸다.
     * 
     * @method isLatestedRevItemReleased
     * @date 2013. 11. 26.
     * @param
     * @return boolean
     * @exception
     ...
     * @see
     */
    public static boolean isLatestedRevItemReleased(TCComponentItemRevision ir) throws Exception {
        if (ir != null) {
            return isReleased(ir);
        } else {
            throw new Exception("itemRevision is null");
        }
    }
     ...
     */
    public static boolean isWritable(TCComponent iComp) throws TCException {
     ...
     * [SR번호][20141217] shcho, NamedReference List만 지우고 실제 imanFile은 (reference 되어있어) 삭제 못하는 경우가 있어 처리 순서를 변경함.
     * [SR번호][20141230] shcho, imanFile을 삭제 못하는 경우 (reference 되어있다는 오류)가 있어, 삭제 방식을 변경하여 dataset.removeFiles API를 사용함.
     * 
     * @param dataset
     *            TCComponentDataset
     * @throws Exception
    ...
    public static void removeAllNamedReference(TCComponentDataset dataset) throws Exception {
        TCComponentTcFile[] imanFile = dataset.getTcFiles();
        String[] arrFileName = new String[imanFile.length];
        String[] arrFileType = new String[imanFile.length];
        for (int j = 0; j < imanFile.length; j++) {
            String orgFileName= imanFile[j].getProperty("original_file_name");
            String objectType =  imanFile[j].getType();
            arrFileName[j] = orgFileName;
            arrFileType[j] = objectType;
        }
        dataset.removeFiles(arrFileName, arrFileType);
...
//        TCComponentTaskTemplate[] tasktemplate = imancomponenttasktemplatetype.extentReadyTemplates(false);
    ...
    /**
     * Argument로 받은 Component의 ACL List를 확인해 허용된 권한의 목록을 문자열 Vector에 담아 Return 한다.
     * 권한의 이름의 예는 다음과 같다.
     * DELETE, READ, WRITE, CHANGE, MARKUP, COPY, CHANGE_OWNER, EXPORT, IMPORT, PUBLISH, DIGITAL_SIGN ...
     * @param component
     * @return
     */
    public static Vector<String> getGrantedPrivilegeNameVector(TCComponent component){
    	
		Vector<String> privilegeNameVector = null;

		if(component==null){
			return privilegeNameVector;
		}
		
		TCAccessControlService accessSvc = null;
		if(component.getSession()!=null){
			accessSvc = component.getSession().getTCAccessControlService();
		}
		
		if(component==null || accessSvc==null){
			return privilegeNameVector;
		}

		privilegeNameVector = new Vector<String>();
		
		try {
			
			ACLLine[] aclLines = accessSvc.getEffectiveACLInfo(component);
			for (int j = 0; j < aclLines.length; j++) {
				
				TCComponent acl = aclLines[j].getACL();
				String aclLineString = aclLines[j].toString();
				TCComponent accessor = aclLines[j].getAccessor();
				
				String accessorType = accessor.getType();
				String accessorClass = accessor.getClass().getName();
				String aclName = acl.getProperty("ACL_Name");
				
				// Accessor Type별로 정의된 ACL을 확인 해야한다.
				//System.out.println("["+j+"] : "+accessorClass+" = "+accessorType+"/"+aclName);
				
				String[] grantedPrivileges = aclLines[j].getGrantedPrivileges();
				
				for (int k = 0; grantedPrivileges!=null && k < grantedPrivileges.length; k++) {
					
					String privilegeStr = grantedPrivileges[k];
					//System.out.println("privilegeStr = "+privilegeStr);
				
					if(privilegeStr!=null && privilegeStr.trim().length()>0){
						if(privilegeNameVector.contains(privilegeStr) == false){
							privilegeNameVector.add(privilegeStr);
						}
					}
				}
			}
...
//		for (int j = 0; privilegeV!=null && j < privilegeV.size(); j++) {
//			System.out.println("privilegeV["+j+"] = "+privilegeV.get(j));
//		}
